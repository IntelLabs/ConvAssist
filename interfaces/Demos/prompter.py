...
class PrompterFrame(wx.Frame):
    ...
    def MakeMenuBar(self):
        ...
        self.word_wrap = self.viewMenu.Append(
            self.ID_TOGGLE_WORD_WRAP,
            "&Word wrap",
        ...
        BindMenu(
            self.viewMenu.Append(wx.ID_ZOOM_IN, "L&arger text\tCTRL++"),
            self.OnViewMenuLargerText,
        )
        BindMenu(
            self.viewMenu.Append(wx.ID_ZOOM_OUT, "S&maller text\tCTRL+-"),
            self.OnViewMenuSmallerText,
        )
        self.viewMenu.AppendSeparator()
        self.ID_SHOW_TOOLBAR = wx.NewId()
        self.show_toolbar_view_menu_item = self.viewMenu.Append(
            self.ID_SHOW_TOOLBAR, "Show &Toolbar", "Show Toolbar", wx.ITEM_CHECK
        )
        ...
        BindMenu(
            self.presageMenu.Append(self.ID_PROMPT_ME, "&Prompt me\tCTRL+P"),
            self.OnPresageMenuPromptMe,
        )

        self.ID_TOGGLE_FUNCTION_KEYS_MODE = wx.NewId()
        self.function_mode_presage_menu_item = self.presageMenu.Append(
            self.ID_TOGGLE_FUNCTION_KEYS_MODE,
        ...
        self.autopunctuation_mode_presage_menu_item = self.presageMenu.Append(
            self.ID_TOGGLE_AUTOPUNCTUATION_MODE,
        ...
        BindMenu(
            self.autopunctuation_mode_presage_menu_item,
            self.OnPresageMenuToggleAutopunctuationMode,
        )
    ...
    def MakeToolBar(self):
        ...
        BindTool(
            self.toolbar.AddLabelTool(
                wx.ID_NEW, "New", new_bmp, shortHelp="New", longHelp="New file"
            ),
            self.OnFileMenuNew,
        )
        BindTool(
            self.toolbar.AddLabelTool(
                wx.ID_OPEN, "Open", open_bmp, shortHelp="Open", longHelp="Open file"
            ),
            self.OnFileMenuOpen,
        )
        BindTool(
            self.toolbar.AddLabelTool(
                wx.ID_SAVE, "Save", save_bmp, shortHelp="Save", longHelp="Save file"
            ),
            self.OnFileMenuSave,
        )
        ...
        BindTool(
            self.toolbar.AddLabelTool(
                wx.ID_CUT, "Cut", cut_bmp, shortHelp="Cut", longHelp="Cut selection"
            ),
            self.OnEditMenuCut,
        )
        BindTool(
        ...
        BindTool(
            self.toolbar.AddLabelTool(
                ...
                prompt_me_bmp,
                shortHelp="Prompt me",
                longHelp="Prompt me with a prediction",
            ),
            self.OnPresageMenuPromptMe,
        )
...
class PrompterEditor(wx.stc.StyledTextCtrl):
    def __init__(self, parent, config, suggestions):
        ...
        class PrompterPresageCallback(ConvAssist.utilities.callback.Callback):
            ...
            def past_stream(self):
                result = self.editor.GetTextRange(0, self.editor.GetCurrentPos())
                return str(result)

            # def get_future_stream(self):
            def future_stream(self):
                result = self.editor.GetTextRange(
                    self.editor.GetCurrentPos(), self.editor.GetTextLength()
                )
                return str(result)
