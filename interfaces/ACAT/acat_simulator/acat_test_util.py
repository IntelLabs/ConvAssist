...
if sys.platform == "win32":
    ...
    set_static_path_param = ConvAssistSetParam(
        ParameterType.PATHSTATIC, base_path + "\\resources\\static_resources"
    )
    set_personalized_path_param = ConvAssistSetParam(
        ParameterType.PATHPERSONALIZED, base_path + "\\resources\\personalized"
    )
        ...
        while not breakloop:
            ...
            if cmd.lower() == "learn":
                for learn_type in list(ConvAssistMessageTypes)[6:9]:
                    msgs.append(
                        ConvAssistMessage(
                            learn_type, ConvAssistPredictionTypes.NONE, args[0]
                        ).jsonSerialize()
                    )
